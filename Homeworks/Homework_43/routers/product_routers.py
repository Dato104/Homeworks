...
async def create_user(data: ProductRequestSchema, db: AsyncSession = Depends(get_db)):
...
async def update_user(product_id: int, data: ProductRequestSchema, db: AsyncSession = Depends(get_db)):
    product_serv = ProductService(db)
    product = await product_serv.update_product(product_id, data)
    return product

@router.delete("/{product_id}", response_model=ProductResponseSchema)
async def delete_user(product_id: int, db: AsyncSession = Depends(get_db)):
    product_serv = ProductService(db)
